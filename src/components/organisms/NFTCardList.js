...
export default function NFTCardList ({ nfts, setNfts, withCreateNFT }) {
  ...
  return (
    ...
    >
      <Grid container className={classNames.grid} id="grid">
        {withCreateNFT && <Grid item xs={12} sm={6} md={3} className={classNames.gridItem}>
          <NFTCardCreation addNFTToList={addNFTToList}/>
        </Grid>}
        {nfts.map((nft, i) =>
          <Fade in={true} key={i}>
            <Grid item xs={12} sm={6} md={3} className={classNames.gridItem} >
                <NFT nft={nft} index={i} />
            </Grid>
          </Fade>
        )}
