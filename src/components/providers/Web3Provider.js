...
export default function Web3Provider ({ children }) {
  ...
  async function initializeWeb3 () {
    try {
      ...
      setHasWeb3(true)
      const provider = new ethers.providers.Web3Provider(WalletConnection, 'any')
      await getAndSetWeb3ContextWithSigner(provider)

      function onAccountsChanged (accounts) {
        // Workaround to accountsChanged metamask mobile bug
        if (onAccountsChangedCooldown || accounts.length < 1) return
      ...
      WalletConnection.on('accountsChanged', onAccountsChanged)
      WalletConnection.on('chainChanged', initializeWeb3)
    } catch (error) {
  ...
  async function setupContracts (signer, networkName) {
    ...
    if(!data.marketplaceAddress || !data.nftAddress) {
      return false
    }
